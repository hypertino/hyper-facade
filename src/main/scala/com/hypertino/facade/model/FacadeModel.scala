...
import com.hypertino.hyperbus.serialization.{ResponseHeader, StringDeserializer}
...
case class FacadeRequest(uri: String, method: String, headersObj: Obj, body: Value) extends FacadeMessage {
  lazy val hri: HRI = HRI.fromURI(uri, PlainHRIQueryConverter)

  def toDynamicRequest: DynamicRequest = {
    DynamicRequest(hri, method, DynamicBody(body), headersObj)
  }
